define(['helper'], function (helper) {
  ...
  self.drawNode = function drawNode(d) {
    ...
    var batmanDomain = 'ffmd';

    if (d.o.domain === batmanDomain) {
      if (d.o.is_online) {
        ctx.arc(d.x, d.y, 8, 0, 2 * Math.PI);
        if (d.o.is_gateway) {
          ctx.rect(d.x - 9, d.y - 9, 18, 18);
        }
        if (d.o.gateway_nexthop === d.o.gateway && !d.o.is_gateway) {
          ctx.fillStyle = config.forceGraph.nodeBatmanUplinkColor;
        } else {
          ctx.fillStyle = config.forceGraph.nodeBatmanColor;
        }
      } else {
        ctx.arc(d.x, d.y, 6, 0, 2 * Math.PI);
        ctx.fillStyle = config.forceGraph.nodeBatmanOfflineColor;
      }
    } else {
      if (d.o.is_online) {
        ctx.arc(d.x, d.y, 8, 0, 2 * Math.PI);

...
define({
  ...
  linkColor: function linkColor(d, linkDirection) {
    var cableLinkColor = '#50b0f0';
    var vpnLinkColor = '#777777';
    if (d.vpn) {
      return vpnLinkColor;
    }
    if (d.type === 'other') {
      return cableLinkColor;
    }

    if (linkDirection === 'source') {
      return require('d3-interpolate').interpolate('#F02311', '#04c714')(d.source_tq);
    }
    if (linkDirection === 'target') {
      return require('d3-interpolate').interpolate('#F02311', '#04c714')(d.target_tq);
    }

    return require('d3-interpolate').interpolate('#F02311', '#04c714')((d.source_tq + d.target_tq) / 2 );
  },
